...
image::arch-mcp.svg[Architecture with custom MCP servers highlighted]

_In this module you work with the highlighted piece: a custom MCP server you deploy and register with OpenShift Lightspeed. Note how it sits alongside the built-in one._
...
IMPORTANT: The trailing slash at the end of the `url` is required. Without it, the MCP server answers with an HTTP redirect that OpenShift Lightspeed does not follow, tool discovery fails, and the model will never see the Insights tools.
...
NOTE: The Insights Result MCP server used in this lab does not require authentication from OpenShift Lightspeed, but the `mcpServers` configuration also supports sending custom HTTP headers to servers that do. Header values can come from a Secret, from the logged-in user's own Kubernetes token, or from the connecting client — so you can integrate MCP servers that enforce their own authentication and authorization.
